...
import { Heart, MessageCircle, Send, MoreHorizontal, Pencil, Trash2, Eye, ChevronDown, ChevronUp, Repeat2, Quote, Bookmark, Link, Share2, Mail, X, Sparkles, ExternalLink, ChevronLeft, ChevronRight, Play, DollarSign, Coins, Lock } from 'lucide-react';
...
const PostCard: React.FC<PostCardProps> = ({ 
  ...
  const handleSendTip = (amount: number) => {
    alert(`Tipped ${amount} Gold to ${post.user.name}!`);
    setShowTipMenu(false);
  };
  ...
  const renderMedia = () => {
    if (displayPost.isLocked) {
        return (
            <div className="mt-3 relative rounded-2xl overflow-hidden border border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-nexus-800 h-72 flex flex-col items-center justify-center p-8 text-center">
                <div 
                    className="absolute inset-0 bg-cover bg-center blur-2xl opacity-40 scale-110"
                    style={{ backgroundImage: `url(${displayPost.imageUrls?.[0] || displayPost.user.avatar})` }}
                ></div>
                <div className="relative z-10">
                    <div className="p-4 bg-nexus-primary/10 rounded-full w-fit mx-auto mb-4">
                        <Lock className="w-8 h-8 text-nexus-primary" />
                    </div>
                    <h4 className="font-black text-lg mb-2 text-gray-900 dark:text-white">Subscriber Exclusive</h4>
                    <p className="text-sm text-gray-500 mb-6 px-4">Subscribe to {displayPost.user.name} for ${displayPost.user.subscriptionPrice}/mo to unlock this post and full catalog.</p>
                    <button 
                        onClick={(e) => { e.stopPropagation(); onUserClick(displayPost.user); }}
                        className="bg-nexus-primary text-white font-black py-3 px-8 rounded-full hover:scale-105 transition-transform shadow-lg shadow-nexus-primary/20"
                    >
                        Subscribe to Unlock
                    </button>
                </div>
    ...
    if (displayPost.videoUrl) {
      return (
        <div className="mt-3 rounded-2xl overflow-hidden border border-gray-100 dark:border-gray-800 bg-neutral-900 shadow-sm relative h-[500px] w-full cursor-pointer">
          <video src={displayPost.videoUrl} className="w-full h-full object-cover" playsInline loop />
        </div>
      );
    }
    ...
    return (
        <div className="mt-3 rounded-2xl overflow-hidden border border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-nexus-800 shadow-sm">
          <img src={images[0]} alt="Post image" className="w-full h-auto max-h-[800px] object-cover" />
        </div>
    );
  };

  return (
    <div className={`border-b border-gray-200 dark:border-gray-800 p-4 transition-all duration-300 ${displayPost.isSponsored ? 'bg-nexus-primary/[0.04]' : ''}`}>
      ...
      {displayPost.isExclusive && !isNested && (
      ...
      <div className="flex flex-col">
        <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-3">
                <div onClick={handleUserClick} className="cursor-pointer shrink-0">
                    <img src={displayPost.user.avatar} alt={displayPost.user.name} className="w-10 h-10 rounded-full object-cover border border-gray-100 dark:border-gray-800" />
                </div>
                <div className="flex flex-col min-w-0">
                  <div className="flex items-center gap-1.5">
                    ...
                    {displayPost.user.isMonetized && !displayPost.isSponsored && (
                        <DollarSign className="w-3.5 h-3.5 text-nexus-primary" />
                    )}
                  </div>
                  <span onClick={handleUserClick} className="text-gray-500 dark:text-gray-400 truncate cursor-pointer text-xs">
        ...
        <div className="flex-1 min-w-0">
          <div className={`mt-1 whitespace-pre-wrap ${isNested ? 'text-sm' : 'text-[16px]'} leading-relaxed text-gray-900 dark:text-gray-100 ${displayPost.isLocked ? 'line-clamp-2 blur-[1px]' : ''}`}>
              {displayPost.content}
          </div>

          {renderMedia()}

          {isOwner && post.adRevenue !== undefined && !isNested && (
